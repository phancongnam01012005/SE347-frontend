...
function ChartTooltipContent({
  ...
  return (
    <div className={cn("border-border/50 bg-white grid min-w-[8rem] items-start gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs shadow-xl", className)}>
      ...
      <div className="grid gap-1.5">
        {payload.map((item, index) => {
          ...
          return (
            <div key={item.dataKey} className={cn("flex w-full items-center gap-2", indicator === "dot" && "items-center")}>
              {formatter && item?.value !== undefined && item.name ? (
                ...
                <>
                  ...
                  <div className={cn("flex flex-1 justify-between leading-none", nestLabel ? "items-end" : "items-center")}>
                    ...
                    {item.value && (
                      <span className="text-gray-900 font-mono font-medium tabular-nums ml-2">
